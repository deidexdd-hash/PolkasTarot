...
const routes=['study','train','compare','stats','deck','modal','book','chapter','cards','lessons','pairs','courts','cases','lenormand','practices','layouts','guide','manual','reading','daily','mirror','journal','library','course','lesson','step','list','card-detail','linked-practice','creator'];
(async()=>{
  ...
  try{
   ...
   for(const width of [320,390,768,1440]){
    ...
    for(const route of routes){
     ...
     if([390,1440].includes(width)&&['study','deck','modal','book','chapter','cards','layouts','reading','daily','journal','library','course','lesson','step','list','linked-practice','creator'].includes(route)){
    ...
    await page.evaluate(async()=>{await Book.open('13',1);await Author.open('lessons','1');App.showHome();});
    assert(await page.locator('#continueWork').isVisible());
    assert.equal(await page.locator('.continue-card').count(),3);
    assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth));
    await page.waitForTimeout(350);await page.evaluate(()=>scrollTo(0,0));
    if([390,1440].includes(width))await page.screenshot({path:path.join(out,`${engineName}-${width}-continue-home.png`)});
