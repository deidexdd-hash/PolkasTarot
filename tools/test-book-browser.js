...
(async()=>{
 ...
 await page.evaluate(()=>Book.open());await page.waitForSelector('.book-row');assert.equal(await page.locator('#bookContents .book-row').count(),36);
 ...
 await page.evaluate(()=>Author.open());await page.waitForSelector('#authorResults .book-row');
 assert.equal(await page.locator('#authorResults .book-row').count(),78);
 await page.locator('#authorSuit').selectOption('wands');assert.equal(await page.locator('#authorResults .book-row').count(),14);
 await page.evaluate(()=>Author.open('cards','minor_arcana.wands.ten'));await page.waitForSelector('.author-title');
 await page.getByRole('button',{name:'Моя практика с картой',exact:true}).click();
 assert.equal(await page.locator('#mirrorCard').inputValue(),'minor_arcana.wands.ten');
 assert(await page.locator('.author-practice-guide').isVisible());
 await page.locator('#mirror-notes').fill('Попробую передоговориться о сроке');
 await page.reload();await page.waitForFunction(()=>DeckLoader.loaded);await page.evaluate(()=>Journal.open(0));
 assert(await page.locator('.author-practice-guide').isVisible());
 assert.equal(await page.locator('#mirror-notes').inputValue(),'Попробую передоговориться о сроке');
 await page.goto('http://127.0.0.1:8765/#study=lenormand&id=4');await page.waitForSelector('.author-title');
 assert.equal(await page.locator('.author-title').textContent(),'Дом');
 for(const width of [320,390,768,1280]){
  await page.setViewportSize({width,height:844});
  assert(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth),'Author surface overflow at '+width);
 }
