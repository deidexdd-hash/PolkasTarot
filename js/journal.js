...
window.Journal = {
    ...
    validate(item) {
        ...
        return ['notes','tags','question','spreadName','date','day','firstLook','followUp','moodBefore','moodAfter'].every(k => item[k] == null || (typeof item[k] === 'string' && item[k].length <= 10000));
    },
...
UI.renderHistory = function() {
    ...
    const items = State.history.map((item,index)=>({item,index})).filter(({item})=>(!favorite || item.favorite) &&
        [item.question,item.notes,item.firstLook,item.followUp,item.moodBefore,item.moodAfter,item.tags,item.spreadName,item.name,...(Array.isArray(item.cards)?item.cards.map(c=>c.name):[])].join(' ').toLocaleLowerCase().includes(query));
