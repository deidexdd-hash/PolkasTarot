...
window.Book = {
    ...
    track(){if(!document.querySelector('.book-prose')||!this.current||document.getElementById('cardModal')?.classList.contains('active'))return;const blocks=[...document.querySelectorAll('[data-book-block]')];const b=blocks.find(p=>p.getBoundingClientRect().bottom>120);if(b){this.position.block=Number(b.dataset.bookBlock);this.persist();this.route(this.current.id,this.position.block);}},
...
Academy.enrich=function(c){bookEnrich(c);const panel=document.querySelector('.card-study');if(panel){const b=document.createElement('button');b.className='book-card-entry text-btn';b.textContent='Карта как зеркало · читать в книге ↗';b.onclick=()=>Book.forCard(c.code);panel.appendChild(b);const full=document.createElement('button');full.className='book-card-entry text-btn';full.textContent='Авторское описание и упражнение ↗';full.onclick=()=>Author.open('cards',c.code);panel.appendChild(full);}};
Book.read();
let bookScrollTimer;
